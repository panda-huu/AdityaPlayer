...
# Wider net used only to detect "someone tried to make a button here but it
# didn't parse" so we can warn the admin at /setwelcome time instead of
# silently saving broken text that every future joiner will see.
_BTN_ATTEMPT_RE = re.compile(r"button\s*url", re.IGNORECASE)


def _normalize_url(url: str) -> str:
    url = url.strip().rstrip(")")
    # fix "https//..." / "http//..." (missing colon after the scheme)
    url = re.sub(r"^(https?):?/{1,2}", lambda m: m.group(1) + "://", url, flags=re.IGNORECASE)
    # no scheme at all e.g. "t.me/xyz" -> assume https
...
def parse_buttons(text: str):
    """Returns (clean_text, buttons, unparsed_count).

    unparsed_count is >0 when the text contains a "buttonurl" attempt that
    _BTN_RE still couldn't match (e.g. broken brackets) — used to warn the
    admin instead of silently saving broken text.
    """
    if not text:
        return text, [], 0
    buttons = []
